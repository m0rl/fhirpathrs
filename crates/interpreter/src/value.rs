...
pub enum Value {
    ...
    Quantity(f64, String, Option<QuantityType>),
...
impl Value {
    ...
    pub fn is(&self, type_spec: &TypeSpecifier) -> bool {
        ...
        match (self, type_name) {
            (Value::Null, _) => false,
            (Value::Number(n), Some("Integer")) => {
                (namespace.is_none() || namespace == Some("System")) && n.fract() == 0.0
            }
            ...
            (Value::Quantity(_, _, Some(qt)), Some(expected)) => {
            ...
            (Value::Number(n), Some("integer" | "positiveInt" | "unsignedInt")) => {
                (namespace.is_none() || namespace == Some("FHIR")) && n.fract() == 0.0
            }
            (Value::Number(_), Some("decimal")) => namespace.is_none() || namespace == Some("FHIR"),
    ...
    pub fn type_name(&self) -> Option<&str> {
        match self {
            ...
            Value::Boolean(_) => Some("Boolean"),
            Value::String(_) => Some("String"),
            Value::Number(n) if n.fract() == 0.0 => Some("Integer"),
            Value::Number(_) => Some("Decimal"),
            Value::Date(..) => Some("Date"),
            Value::DateTime(..) => Some("DateTime"),
            Value::Time(..) => Some("Time"),
            Value::Quantity(_, _, t) => Some(t.map_or("Quantity", QuantityType::as_str)),
    ...
    pub fn as_type(&self, type_spec: &TypeSpecifier) -> Value {
        ...
        match (self, parts.last().map(|s| s.as_str())) {
            ...
            (Value::String(s), Some("Integer")) => s
                ...
                .parse::<f64>()
                .ok()
                .map_or_else(|| Value::collection(vec![]), |n| Value::Number(n.trunc())),
            (Value::String(s), Some("Decimal")) => s
                .trim()
                .parse::<f64>()
                .ok()
                .map_or_else(|| Value::collection(vec![]), Value::Number),
            ...
            (Value::Number(n), Some("String")) => Value::String(n.to_string()),
            (Value::Number(n), Some("Integer")) => Value::Number(n.trunc()),
            (Value::Number(n), Some("Boolean")) => {
                if *n == 1.0 {
                    Value::Boolean(true)
                } else if *n == 0.0 {
                    Value::Boolean(false)
            ...
            (Value::Quantity(v, u, _), Some("String")) => Value::String(format!("{} '{}'", v, u)),
            (Value::Quantity(v, ..), Some("Decimal")) => Value::Number(*v),
            (Value::Quantity(v, ..), Some("Integer")) => Value::Number(v.trunc()),
            _ => Value::collection(vec![]),
        }
    }
        ...
        loop {
            match val {
                ...
                Value::Object(obj) => {
                    ...
                    return Some(Value::Quantity(v, code, qt));
                }
                _ => return None,
            }
        }
    ...
    pub fn equals(&self, other: &Value) -> bool {
        ...
        while let Some((a, b)) = stack.pop() {
            if !match (a, b) {
                ...
                (Value::Number(na), Value::Number(nb)) => (na - nb).abs() < f64::EPSILON,
                ...
                (Value::Quantity(v1, u1, qt1), Value::Quantity(v2, u2, qt2)) => {
    ...
    fn compare_equivalent(&self, other: &Value) -> Option<std::cmp::Ordering> {
        ...
        while let Some((a, b)) = stack.pop() {
            let ord = match (a, b) {
                ...
                (Value::Quantity(v1, u1, qt1), Value::Quantity(v2, u2, qt2)) => v1
                    .total_cmp(v2)
...
impl std::fmt::Display for Value {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Value::Quantity(v, u, _) => write!(f, "{} '{}'", v, u),
